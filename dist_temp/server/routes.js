...
import { answerSchema, powerUpSchema, loginUserSchema } from "@shared/schema";
...
export async function registerRoutes(app) {
    ...
    app.get("/api/questions/categories", async (req, res) => {
        try {
            ...
            const categories = await storage.getAllCategories(challengeType);
            res.json(categories);
        }
        catch (error) {
    ...
    app.get("/api/questions/count-by-category", async (req, res) => {
        try {
            const { challengeType } = req.query;
            const categoryCounts = await storage.getQuestionCountByCategory(challengeType);
    ...
    app.get("/api/user/:userId/stats", async (req, res) => {
        try {
            ...
            const { challengeType } = req.query;
            const stats = await storage.getUserStats(userId, challengeType);
            res.json(stats);
        }
        catch (error) {
    ...
    app.get("/api/user/:userId/answers", async (req, res) => {
        try {
            ...
            const answers = await storage.getUserAnswers(userId, limit ? parseInt(limit) : undefined);
            res.json(answers);
        }
        catch (error) {
    ...
    app.get("/api/admin/export/questions", isAdmin, async (req, res) => {
        try {
            ...
            if (category && challengeType) {
                questions = await storage.getRandomQuestions(1000, challengeType, category);
            }
            else if (challengeType) {
                questions = await storage.getRandomQuestions(1000, challengeType);
            }
            else {
